...
esp_err_t initCellular(enum supportedModems modem, bool hardReset) {
    ...
    if (modem_netif_adapter == NULL) return ESP_FAIL;
    ...
    if (hardReset) {
        ...
        pinCfg.pin_bit_mask = (1UL << PWR_PIN);
    ...
    for (size_t k = 0; k < 15; k++) {
        ...
        if (dce->attached == ATTACH_ROAMING ||
            dce->attached == ATTACH_HOME_NETWORK ||
            (dce->attached == ATTACH_NOT_SEARCHING && k > 4))
            break;
    ...
    //     {
    //     case 0: //GSM
    //         ESP_LOGI("Network: ", "Type: GSM, Name: %s, mccmnc: %s",
    //         net->name, net->mccmnc); break;
    //     case 7: //LTE
    //         ESP_LOGI("Network: ", "Type: LTE-M, Name: %s, mccmnc: %s",
    //         net->name, net->mccmnc); break;
    ...
    if (!(dce->attached == ATTACH_ROAMING ||
          dce->attached == ATTACH_HOME_NETWORK)) {  // at+cops = 5/0
        dce->attach(dce, 1);
    }
    ...
    while (!(dce->attached == ATTACH_ROAMING ||
             dce->attached == ATTACH_HOME_NETWORK)) {
        if (dce->checkNetwork(dce) != ESP_OK) errorCount++;

        if (errorCount > 20) return ESP_FAIL;
        ...
        switch (dce->attached) {
            ...
            case ATTACH_ROAMING:
                ...
                break;
            case ATTACH_DENIED:
                // we might get this in case of network whitelist. The device
                // can connect to other networks though!
    ...
    if (rssi > 90) dce->get_signal_quality(dce, &rssi, &ber);

    ESP_LOGI(TAG, "rssi: %d, ber: %d", rssi, ber);
    signalQuality = rssi;

    // setup psm and edrx
    // In this case PSM does not add any value, as the modem is quite slow to
    // boot. Furthermore, we can tear down the connection quicker by toggling
    // power with the powerpin.
    ...
    uxBits = xEventGroupWaitBits(event_group, CONNECT_BIT, pdTRUE, pdTRUE,
                                 xTicksToWait);
    if (!(uxBits & CONNECT_BIT)) return ESP_FAIL;
    initialized = 1;
    return ESP_OK;
}
...
esp_err_t forcePowerDown() {
    ...
    if (dce->PSM) {
        ...
        if (dce && dte) {
            ...
            xEventGroupWaitBits(event_group, STOP_BIT, pdTRUE, pdTRUE,
                                portMAX_DELAY);
    ...
    // no psm or we never get the psm enter notification. Toggle pin to power
    // down.. bit harder stop, but modem gets to detach as needed.
    dce->power_down(dce);
    return ESP_OK;
}