...
enum EVENT_BITS { TOUCH_EVENT = BIT0 };

enum NET_STATUS { DETACHED, ATTACHED, IP, TRANSMITTING };
...
void app_main(void) {
    ...
    // esp_pm_config_esp32_t conf = {.max_freq_mhz = 40, .min_freq_mhz = 40,
    // .light_sleep_enable = 0}; esp_pm_configure(&conf);
    ...
    xTaskCreate(watchdog_task, "WATCHDOG_TASK", 4000, NULL, tskIDLE_PRIORITY,
                &xHandleWatchdog);
    ...
    // initialize the cellular connection. Registers the PPPoS with on the
    // network stack.

    uint8_t shouldDoHardReset = app_state.failed_init_count > 3 ? 1 : 0;

    if (shouldDoHardReset) {
        app_state.failed_init_count = 0;
    }

    esp_err_t status = initCellular(SIM800, shouldDoHardReset);

    if (status != ESP_OK) {
        app_state.failed_init_count++;
        fault_state();
    } else {
        app_state.failed_init_count = 0;
    }
    ...
    do {
        ...
        if (!postErr) vTaskDelay(pdMS_TO_TICKS(20000));
    ...
    //     // uxBits = xEventGroupWaitBits(eventGroup, TOUCH_EVENT, pdTRUE,
    //     pdTRUE, xTicksToWait);
    ...
    //         // sprintf(payload, "{\"battery\": %f,\"signal\":
    //         %f,\"temperature\": %f}", battery, signal, temp);
...
void powerOff(uint32_t RTCSleepInS) {
    ...
    if (RTCSleepInS) {
        esp_sleep_enable_timer_wakeup((uint64_t)RTCSleepInS * (uint64_t)1000 *
                                      (uint64_t)1000);  // 10 secs
    }
...
void init_adc() {
    ...
    adc_power_acquire();
    adc_char = calloc(1, sizeof(esp_adc_cal_characteristics_t));
    esp_adc_cal_characterize(ADC_UNIT_1, ADC_ATTEN_11db, ADC_WIDTH_BIT_12, 1,
                             adc_char);
    adc1_config_width(ADC_WIDTH_BIT_12);
...
float get_batt_voltage() {
    ...
    return (float)esp_adc_cal_raw_to_voltage(raw, adc_char) * 2 /
           1000.0;  // x2 due to voltage division..
}
...
void watchdog_task(void *param) {
    ...
    // -> this should hopefully never happen, but if the socket API stalls this
    // should 'handle' it.
...
static void test_on_ping_timeout(esp_ping_handle_t hdl, void *args) {
    ...
    if (app_state.timeout_count > 1) app_state.error_state = 1;
}
...
void initialize_ping() {
    ...
    cbs.cb_args = NULL;  // arguments that will feed to all callback functions,
                         // can be NULL
